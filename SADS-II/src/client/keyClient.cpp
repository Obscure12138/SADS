...
u_char r[32] = { 0x72, 0x12, 0x8a, 0x7a, 0x17, 0x52, 0x6e, 0xbf,
...
void KeyClient::run()
{
    ...
    u_char chunkHash[CHUNK_HASH_SIZE * keyBatchSize_ * 2];
    bool JobDoneFlag = false;
    NetworkHeadStruct_t dataHead;
    ...
    while (true) {
        ...
        if (extractMQ(tempChunk)) {
            ...
            memcpy(chunkHash + batchNumber * CHUNK_HASH_SIZE * 2, tempChunk.chunk.feature, CHUNK_HASH_SIZE);
            memcpy(chunkHash + batchNumber * CHUNK_HASH_SIZE * 2 + CHUNK_HASH_SIZE, tempChunk.chunk.chunkHash, CHUNK_HASH_SIZE);
            batchNumber++;
        }
...
{
    u_char sendHash[CHUNK_HASH_SIZE * batchNumber * 2 + 32];
#if SYSTEM_BREAK_DOWN == 1
    struct timeval timestartKey_enc;
    struct timeval timeendKey_enc;
    gettimeofday(&timestartKey_enc, NULL);
#endif
	memcpy(sendHash,batchHashList,CHUNK_HASH_SIZE * batchNumber * 2);
    memcpy(sendHash + CHUNK_HASH_SIZE * batchNumber *2, r, 32); 
    cryptoObj_->keyExchangeEncrypt(sendHash, batchNumber * CHUNK_HASH_SIZE * 2 + 32, keyExchangeKey_, keyExchangeKey_, sendHash);
    //PRINT_BYTE_ARRAY_KEY_CLIENT(stdout,sendHash + batchNumber*CHUNK_HASH_SIZE*2,32);
#if SYSTEM_BREAK_DOWN == 1
...
#endif
    if (!keySecurityChannel_->send(sslConnection_, (char*)sendHash, CHUNK_HASH_SIZE * batchNumber *2 + 32)) {
    ...
    gettimeofday(&timestartKey_enc, NULL);
#endif
    cryptoObj_->keyExchangeDecrypt(recvBuffer, batchkeyNumber * CHUNK_ENCRYPT_KEY_SIZE + 32, keyExchangeKey_, keyExchangeKey_, recvBuffer);
    //PRINT_BYTE_ARRAY_KEY_CLIENT(stdout,recvBuffer+batchkeyNumber * CHUNK_ENCRYPT_KEY_SIZE, 32);
    if (memcmp(r, recvBuffer + batchNumber * CHUNK_HASH_SIZE, 32) != 0) {
        //cerr << "KeyClient : recved keys r error" << endl;
#if SYSTEM_DEBUG_FLAG == 1
	...
	memcpy(batchKeyList,recvBuffer,CHUNK_HASH_SIZE * batchNumber);
#if SYSTEM_BREAK_DOWN == 1
    gettimeofday(&timeendKey_enc, NULL);
