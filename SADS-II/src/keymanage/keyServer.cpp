...
bool keyServer::runRemoteAttestationInit()
{
    ...
    char sessionKey_[64];
    client->GetSessionKey(sessionKey_);
    raSecurityChannelTemp->send(sslConnection, sessionKey_, 32);
    cout<<"KeyServer : send session key success"<<endl;
    delete raSecurityChannelTemp;
    free(sslConnection);
    // if (remoteAttestationStatus) {
    //     delete raSecurityChannelTemp;
...
void keyServer::runKeyGenerateThread(SSL* connection)
{
    ...
    while (true) {
        u_char hash[config.getKeyBatchSize() * CHUNK_HASH_SIZE * 2 + 32];
        int recvSize = 0;
        ...
        //PRINT_BYTE_ARRAY_KEY_SERVER(stdout,hash+recvSize - 32, 32);
        int recvNumber = (recvSize - 32) / (CHUNK_HASH_SIZE * 2);
        if (recvNumber == 0) {
            multiThreadCountMutex_.lock();
            clientThreadCount_--;
#if SYSTEM_BREAK_DOWN == 1
...
#endif
        u_char key[recvNumber * CHUNK_ENCRYPT_KEY_SIZE + 32];
#if SYSTEM_BREAK_DOWN == 1
        gettimeofday(&timestart, 0);
#endif
        client->request(hash, recvSize, key, recvNumber * CHUNK_ENCRYPT_KEY_SIZE + 32);
        //PRINT_BYTE_ARRAY_KEY_SERVER(stdout, key+recvNumber * CHUNK_HASH_SIZE, 32);
#if SYSTEM_BREAK_DOWN == 1
        gettimeofday(&timeend, 0);
        diff = 1000000 * (timeend.tv_sec - timestart.tv_sec) + timeend.tv_usec - timestart.tv_usec;
        second = diff / 1000000.0;
