...
void DataSR::runKeyServerRemoteAttestation()
{
    ...
    char keyExchangeKey[64];
	int recvSize;
    sslRAListen->recv(sslRAListenConnection, keyExchangeKey, recvSize);
    memcpy(keyExchangeKey_,keyExchangeKey,32);
	keyExchangeKeySetFlag_ = true;
    cerr << "DataSR : recv session key" << endl;
