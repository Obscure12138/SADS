...
sgx_status_t ecall_keygen(uint8_t* src, uint32_t srcLen, uint8_t* key)
{
    ...
    uint8_t hash[32], originhash[srcLen],hashTemp[96];
    uint32_t keyLen = ((srcLen - 32) / 2) + 32;
    if (!decrypt(src, srcLen, currentSessionKey_, 32, originhash, &decryptLen)) {
        return SGX_ERROR_UNEXPECTED;
    } else {
        for (uint32_t index = 0; index < ((srcLen -32) / 64); index++) {
            memcpy_s(hashTemp, 96, originhash + index * 64, 64);
            memcpy_s(hashTemp + 64, 96, serverSecret_, 32);
            sgx_status_t sha256Status = sgx_sha256_msg(hashTemp, 96, (sgx_sha256_hash_t*)hash);
            if (sha256Status != SGX_SUCCESS) {
                return sha256Status;
            } else {
                memcpy_s(key + index * 32, keyLen, hash, 32);
            }
			memcpy_s(key + keyLen - 32, keyLen, src + srcLen - 32, 32);
        }
        if (!encrypt(key, keyLen, currentSessionKey_, 32, key, &encryptLen)) {
            return SGX_ERROR_UNEXPECTED;
        } 
    }
}
